# -*- coding: utf-8 -*-
"""
Created on Tue May  4 15:40:45 2021
...
# train_y = to_categorical(train_y, number_of_category)
# val_y = to_categorical(val_y, number_of_category)
# test_y = to_categorical(test_y, number_of_category)
...
# In[] Train dynamic model
# model_D = LeNet_D(num_classes = number_of_category)

# model_D.compile(loss=tf.losses.categorical_crossentropy,
#               optimizer=tf.optimizers.SGD(learning_rate=1e-1, decay=1e-6, momentum=9e-1),
#               metrics=['accuracy'])

# start = time.time()
# history_D = model_D.fit(train_dataset,
#                      epochs = 10,
#                      validation_data=validation_dataset,
#                      verbose = 2)
...
# B = model_D.evaluate(test_dataset)
